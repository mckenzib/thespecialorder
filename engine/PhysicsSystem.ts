...
export class PhysicsSystem {
    ...
    public update(state: GameState) {
        ...
        entities.forEach(entity => {
            if (entity.type === EntityType.PLATFORM_MOVING) {
                // Update Position
                entity.pos.x += entity.vel.x;
                entity.pos.y += entity.vel.y;

                // Reverse Direction logic
                if (entity.moveAxis === 'x') {
                    const dist = entity.pos.x - (entity.startPos?.x || 0);
                    if (Math.abs(dist) > (entity.moveRange || 100)) {
                        entity.vel.x *= -1;
                    }
                } else {
                    const dist = entity.pos.y - (entity.startPos?.y || 0);
                    if (Math.abs(dist) > (entity.moveRange || 100)) {
                        entity.vel.y *= -1;
                    }
                }
        ...
        entities.forEach(entity => {
            if ([EntityType.ENEMY_ONION, EntityType.ENEMY_CILANTRO, EntityType.ENEMY_SALT].includes(entity.type)) {
                ...
                state.entities.forEach(platform => {
                    if ([EntityType.PLATFORM, EntityType.PLATFORM_CLOUD, EntityType.PLATFORM_MOVING].includes(platform.type) && this.checkCollision(entity, platform)) {
    ...
    public resolvePlatformCollision(entity: Entity, platform: Entity) {
        // CLOUD PLATFORM LOGIC (One-Way)
        if (platform.type === EntityType.PLATFORM_CLOUD) {
            // Only collide if falling AND feet are above the platform center (roughly)
            // We use previous position logic implicitly by checking overlap depth
            const playerBottom = entity.pos.y + entity.size.y;
            const platformTop = platform.pos.y;

            // If we are moving up (jumping), ignore collision
            if (entity.vel.y < 0) return;

            // If we are already too deep inside, ignore (allows jumping through)
            if (playerBottom > platformTop + 10) return;

            // Snap to top
            entity.pos.y = platformTop - entity.size.y;
            entity.vel.y = 0;
